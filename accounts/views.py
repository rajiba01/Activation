...
def send_login_alert_email(user, ip, geo, timestamp):
    logger.info("=== DEBUT send_login_alert_email ===")  # <-- LOG 1
    logger.info("user.email = %s", user.email)  # <-- LOG 2
    ...
    logger.info("Subject: %s", subject)  # <-- LOG 3
    logger.info("DEFAULT_FROM_EMAIL: %s", settings.DEFAULT_FROM_EMAIL)  # <-- LOG 4
    logger.info("EMAIL_BACKEND: %s", settings.EMAIL_BACKEND)  # <-- LOG 5
    
    try:
        logger.info("Envoi send_mail en cours...")  # <-- LOG 6
        result = send_mail(subject, message, settings.DEFAULT_FROM_EMAIL, [user.email], fail_silently=False)
        logger.info("send_mail réussi, result=%s", result)  # <-- LOG 7
        return True
    except Exception as e:
        logger.exception("EXCEPTION dans send_mail: %s", e)  # <-- LOG 8
        return False
...
def register_user(request):
    ...
    if serializer.is_valid():
        user = serializer.save()  # serializer.create doit retourner l'instance user
        ...
        # construction du lien d'activation (adapter le chemin si besoin)
...
def geolocate_ip(ip):
    try:
        ...
        # pour localhost, demander info générique
...
@permission_classes([permissions.AllowAny])
def login(request):
    logger.info("=== LOGIN CALLED ===")  # <-- TEST
    username = request.data.get('username')
    password = request.data.get('password')
    user = authenticate(username=username, password=password)
    if user is None:
        return Response({'status': 'error', 'message': 'Username ou password incorrect'}, status=status.HTTP_401_UNAUTHORIZED)

    from rest_framework_simplejwt.tokens import RefreshToken
    refresh = RefreshToken.for_user(user)
    
    logger.info("=== AVANT send_login_alert_email ===")  # <-- TEST 2
    try:
    ...
    email_ok = send_login_alert_email(user, ip, geo, timestamp)  # <-- DOIT APPELER
    logger.info("=== APRES send_login_alert_email, email_ok=%s ===", email_ok)  # <-- TEST 3
