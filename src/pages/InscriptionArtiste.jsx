...
/* ════════════ COMPOSANT PRINCIPAL ════════════ */
export default function InscriptionArtiste() {
  ...
  const navigate = useNavigate();  // 🆕 CORRIGER ICI
  ...
  const handleSubmit = async (e) => {
    ...
    if (view === "register" && step === 2) {
      try {
        ...
        console.log('Données artiste:', userData);

        const result = await register(userData);
        console.log('Inscription artiste réussie:', result);
