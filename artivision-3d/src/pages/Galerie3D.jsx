// src/pages/Galerie3D.jsx — Version avec synchronisation des ventes
// (garde tout votre code, seules les parties AJOUTÉES sont marquées)
...
export default function Galerie3D() {
  ...
  // ========== AJOUT ÉTAPE 3 : Référence pour le polling ==========
  const pollingIntervalRef = useRef(null);

  // ========== AJOUT ÉTAPE 3 : Fonction pour charger les œuvres (avec filtrage vendues) ==========
  const loadArtworks = useCallback(async () => {
    ...
    try {
      ...
      // ========== AJOUT ÉTAPE 3 : Exclure les œuvres vendues ==========
  ...
  // ========== AJOUT ÉTAPE 3 : Fonction pour rafraîchir les œuvres ==========
  ...
  // ========== AJOUT ÉTAPE 3 : Démarrer le polling après chargement ==========
  useEffect(() => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // ========== MODIFICATION ÉTAPE 3 : Utiliser loadArtworks au lieu du chargement direct ==========
  ...
  // Ouvrir le modal de paiement pour une œuvre
  const handleBuyArtwork = (oeuvre) => {
    setPaymentArtwork(oeuvre);
    setShowPaiementModal(true);
  };

  // ========== MODIFICATION ÉTAPE 3 : Rafraîchir après paiement ==========
    ...
    <div style={{ position: "fixed", inset: 0, background: "#0a0508", overflow: "hidden" }}>
      ...
      >
        <Suspense fallback={<Loader3D message="Préparation de la galerie…" />}>
          ...
          {/* Sélection du style de salle selon subscription_type */}
          {roomStyle === "atelier" && (
            <SalleAtelier3D
              chambre={chambre}
              oeuvres={oeuvres}
      ...
      {/* Modal de paiement avec formulaire carte bancaire */}
